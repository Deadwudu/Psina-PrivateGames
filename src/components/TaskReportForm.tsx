  ...
  if (tasks.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-[var(--border)] p-6 text-sm text-[var(--muted)]">
        Нет задач, по которым сейчас можно отправить рапорт: личные без рапорта или общие, по которым ещё не стоит успех.
        Личные появятся после выдачи; общая исчезнет из списка после успешного рапорта по стороне.
      </p>
    );
  }
