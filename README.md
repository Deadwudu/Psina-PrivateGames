...
- **Роли:** две игровые стороны (`side_a` / `side_b` при регистрации; отображаемые названия задаёт админ), **Администратор** (назначается вручную в БД).
- **Главный экран:** Карта полигона, мои задачи, рапорт, взлом двери/сервера, дешифровка.
- **Админ:** выдача задач, правка рапортов, просмотр взломов/дешифровки; переименование сторон; **сброс игровых данных** после мероприятия (рапорты, задачи, взломы, маркеры карты — без удаления игроков).
...
8. Затем `supabase/migrations/007_app_settings_side_names.sql` (отображаемые названия сторон; ключи `side_a_display_name`, `side_b_display_name`).
9. **Settings → API** скопируйте **Project URL** и **service_role** secret (ключ не светите в браузере и не коммитьте).
...
| `app_settings` | Пары ключ–значение (названия сторон: `side_a_display_name`, `side_b_display_name`) |
